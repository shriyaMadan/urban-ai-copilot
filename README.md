...
### Minimal secrets for deployment

At minimum, set these in **App Settings → Secrets**:
...
Optional (recommended for richer demo):
...
- `requirements.txt` contains only runtime dependencies used by the app.
- `app.py` is the single Streamlit entrypoint.
- No local filesystem paths are hardcoded.
- App handles missing keys gracefully with fallback behavior.
- Commit `.streamlit/config.toml` for consistent cloud behavior.

For production-like demos, configure at least:

- `GEMINI_API_KEY` (or OpenAI-compatible keys)
- `TOMTOM_API_KEY` (optional but recommended for richer traffic context)
